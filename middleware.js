...
export async function middleware(request) {
  ...
  //     pathname.startsWith(`/${locale}/orders-history`) ||
  ...
  const isLoginRoute = pathname.startsWith("/login") || pathname.startsWith(`/${locale}/login`);
...
//       return NextResponse.redirect(new URL(`/${nextLocal?.value}/login`, request.url));
//     }
//   }
  ...
  if (isLoginRoute) {
    if (userCookie) {
      ...
      return NextResponse.redirect(new URL(`/${nextLocal.value}/`, request.url));
    }
  }
